        ...
        try:
            while not done_evt.is_set():
                try:
                    ...
                    # Send keepalive while review is running
                    self.wfile.write(b": keepalive\n\n")
                    self.wfile.flush()
            # Review done — drain remaining events and close
            while True:
                try:
                    event = q.get_nowait()
                    self.wfile.write(f"event: {event.kind}\ndata: {event.to_json()}\n\n".encode())
...
class DashboardServer:
    ...
    def start(self) -> str:
        ...
        self._server.RequestHandlerClass._stop_evt = self._stop_evt
    ...
    def _run(self):
        while self._server and not self._stop_evt.is_set():
